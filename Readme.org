...
*** Without XWindows 
 ...
 #+end_src
  ...
  #+begin_src text :tangle (concat *TrampPrefix* *DockerFiles* "Dockerfile-spacemacs_FullDev_Xless")
    ...
    # install XPRA package
    #RUN apt-get install xpra
    RUN apt-get install libx11-dev libxtst-dev libxcomposite-dev libxdamage-dev libxkbfile-dev python-all-dev pandoc libsystemd-dev
    RUN apt-get install xauth x11-xkb-utils libgtk-3-dev python3-dev python3-cairo-dev python-gi-dev cython3 
    RUN apt-get install libpam-dev quilt xserver-xorg-dev xutils-dev xserver-xorg-video-dummy xvfb keyboard-configuration
    RUN apt-get install libturbojpeg-dev libwebp-dev python3-pil
    RUN apt-get install openssh-client sshpass python3-paramiko
    RUN apt-get install python3-setproctitle python3-xdg python3-pyinotify python3-opencv
    RUN apt-get install uglifyjs brotli libjs-jquery libjs-jquery-ui gnome-backgrounds
    RUN apt-get install python3-rencode python3-lz4 python3-dbus python3-cryptography python3-netifaces python3-yaml python3-lzo
    RUN git clone https://github.com/Xpra-org/xpra
    RUN git clone https://github.com/Xpra-org/xpra-html5
    WORKDIR xpra-html5
    RUN ./setup.py install /usr/share/www/xpra
    WORKDIR ../xpra
    ...
    ENTRYPOINT /bin/FixSSHPermitions.sh;bash       
 #+end_src
**** build images    
 #+begin_src bash :term *dockerBuild* :dir (concat *TrampPrefix* *DockerFiles*) :results silent 
   cp $dockerfile Dockerfile
