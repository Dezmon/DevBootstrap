...
* Dockerize
:PROPERTIES:
  :header-args:bash+: :var versionNumber='2.0'
  :END:
** layering
...
** Build emacs
 #+begin_src text :tangle (concat *TrampPrefix* *DockerFiles* "Dockerfile-emacs_base")
   ...
   RUN apt-get install -y make sudo tar unzip 
   RUN apt-get install -y wget apt-utils automake 
   RUN apt-get install -y libjpeg-dev libncurses5-dev libgl1-mesa-glx 
   ...
   RUN apt-get install -y texinfo libtiff5-dev 
   RUN apt-get install  -y webkit2gtk-4.0
...
#+end_src
 ...
 #+begin_src bash :term dockerpush  :dir (concat *TrampPrefix* *DockerFiles*)  :results silent
   docker tag dperrin/emacs_base dperrin/emacs_base:$versionNumber
...
**** build images    
 #+begin_src bash :term *dockerBuild* :dir (concat *TrampPrefix* *DockerFiles*) :results silent 
   ...
   docker build -t dperrin/$dockerName .
   #exit
 #+end_src
...
*** build images
#+begin_src bash :term *dockerBuild*  :dir (concat *TrampPrefix* *DockerFiles*)  :results silent
  cp Dockerfile-latex Dockerfile
  docker build -t dperrin/spacemacs_latex .
#+end_src

 #+begin_src bash  :term dockerpush :dir (concat *TrampPrefix* *DockerFiles*)  :results silent
   docker tag dperrin/spacemacs_latex dperrin/spacemacs_latex:$versionNumber
   docker push dperrin/spacemacs_latex:$versionNumber
 #+end_src
...
*** push to dockerhub
#+begin_src :term *dockerBuild* :dir (concat *TrampPrefix* *DockerFiles*)  :results raw drawer
   docker tag dperrin/$dockerName dperrin/$dockerName:latest
...
#+begin_src bash  :session uploading :dir (concat *TrampPrefix* *DockerFiles*)  :results raw drawer
   docker tag dperrin/spacemacs_literate_datascience:dev dperrin/spacemacs_literate_datascience:$versionNumber
...
**** build images    
 ...
 #+begin_src bash  :term dockerpush :dir (concat *TrampPrefix* *DockerFiles*)  :results silent
   docker tag dperrin/spacemacs_full_dev_xless dperrin/spacemacs_full_dev_xless:$versionNumber
   docker push dperrin/spacemacs_full_dev_xless:$versionNumber
 #+end_src
